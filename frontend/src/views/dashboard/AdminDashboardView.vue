...
async function loadDashboardData() {
  ...
  try {
    ...
    users.value = Array.isArray(usersData) ? usersData.map(normalizeUserForDashboard) : []
  } catch (error) {
    console.error(error)
    loadError.value = 'No hemos podido cargar el panel de administración.'
  }
}
...
function normalizeUserForDashboard(user = {}) {
  return {
    ...
    is_active: user.is_active ?? user.status !== 'inactive',
  }
}
...
function normalizeReservationForDashboard(reservation = {}) {
  return {
    ...
    pet: reservation.pet
      ? {
          ...reservation.pet,
          photo_path: buildStorageUrl(reservation.pet.photo_path || reservation.pet.photo_url || ''),
        }
      : null,
  }
}
...
const activeUsersCount = computed(() => users.value.filter((user) => user.status === 'active' || user.is_active).length)
const inactiveUsersCount = computed(() => users.value.filter((user) => user.status !== 'active' && !user.is_active).length)
...
const roleCards = computed(() => {
  ...
  return roles.map((role) => ({
    ...
    value: users.value.filter((user) => normalizeRole(user.role) === role).length,
  }))
})
...
const resourceStatusCards = computed(() => {
  ...
  return [
    ...
    },
    {
      ...
      value: disabled,
      width: `${Math.max(8, (disabled / total) * 100)}%`,
...
function getReportStatus(reservation) {
  const directReport = reservation.daily_report || reservation.dailyReport || reservation.follow_up || reservation.followUp
  const reports = Array.isArray(reservation.daily_reports)
    ? reservation.daily_reports
    : Array.isArray(reservation.dailyReports)
      ? reservation.dailyReports
      : []

  const report = directReport || reports[0] || null
  ...
  if (reports.length > 0 && !status) return 'published'
  return 'pending'
}
...
function buildStorageUrl(value) {
  ...
  if (String(value).startsWith('/storage/')) return `${API_BASE_URL}${value}`
  if (String(value).startsWith('storage/')) return `${API_BASE_URL}/${value}`
